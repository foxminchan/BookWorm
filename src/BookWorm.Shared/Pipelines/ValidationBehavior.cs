    ...
    where TResponse : notnull
{
    ...
    )
    {
        ...
        var failures = validationResult
            .Select(r =>
                r.Errors.Select(e => new ValidationFailure(e.PropertyName, e.ErrorMessage))
            )
            .SelectMany(f => f)
            .ToList();
        ...
        return await activityScope.Run(
            ...
            new()
            {
                Tags =
                {
                    ...
                    {
                        TelemetryTags.Validator.ValidationRequest,
                        JsonSerializer.Serialize(request)
                    },
                    { TelemetryTags.Validator.ValidationResponseType, typeof(TResponse).FullName },
                    {
                        TelemetryTags.Validator.ValidationValidators,
                        JsonSerializer.Serialize(validators)
                    },
                },
            },
            cancellationToken
