...
internal sealed record OrderSummary(Guid Id, Status Status, decimal TotalPrice)
{
    ...
    {
        return new((Guid)@event.Order.Id, Status.New, @event.Order.TotalPrice);
    }

    public static OrderSummary Apply(OrderCancelledEvent @event)
    {
        return new((Guid)@event.Order.Id, Status.Cancelled, @event.Order.TotalPrice);
    }

    public static OrderSummary Apply(OrderCompletedEvent @event)
    {
        return new((Guid)@event.Order.Id, Status.Completed, @event.Order.TotalPrice);
    }
}
