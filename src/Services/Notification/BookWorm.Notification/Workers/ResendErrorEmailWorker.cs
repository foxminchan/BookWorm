...
) : IHostedService, IDisposable
{
    ...
    private async Task ResendFailedEmails()
    {
        try
        {
            ...
            foreach (var emailId in failedEmails.Select(e => e.Id).ToList())
            {
                try
                {
                    var email = failedEmails.First(e => e.Id == emailId);
                ...
                {
                    logger.LogError(ex, "Failed to resend email with ID {EmailId}", emailId);
                }
            }
        }
        catch (Exception ex)
