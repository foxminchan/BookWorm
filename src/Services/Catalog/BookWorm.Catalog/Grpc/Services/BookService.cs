    ...
    : BookGrpcService.BookGrpcServiceBase
{
    ...
    private static BookResponse MapToBookResponse(Book book)
    {
        var response = new BookResponse
        {
            Id = book.Id.ToString(),
            Name = book.Name,
            Price = DecimalValueExtensions.ToDecimal(book.Price!.OriginalPrice),
            Status = book.Status == Status.InStock ? BookStatus.InStock : BookStatus.OutOfStock,
        };

        // Always set PriceSale - if there's a discount price use it, otherwise use default (0)
        if (book.Price?.DiscountPrice is not null)
        {
            response.PriceSale = DecimalValueExtensions.ToDecimal(book.Price.DiscountPrice.Value);
        }
        else
        {
            response.PriceSale = DecimalValueExtensions.ToDecimal(0m);
        }
