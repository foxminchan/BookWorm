...
public sealed class BasketService(GrpcBasketClient basketClient, ILogger<BasketService> logger)
    : IBasketService
{
    ...
    )
    {
        ...
        {
            logger.LogDebug(
                "[{Service}] - Begin retrieving basket for {BasketId}",
                nameof(BasketService),
                basketId
            );
        }

        var response = await basketClient.GetBasketAsync(
            new(),
    ...
    {
        return new(
            Guid.Parse(basket.BasketId),
            basket.Books.Select(MapBasketItem),
            (decimal)basket.TotalPrice
        );
    }
