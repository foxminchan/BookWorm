...
internal sealed class OrderCreatedIntegrationEventHandler(
    ...
    ILogger<OrderCreatedIntegrationEventHandler> logger) : IConsumer<OrderCreatedIntegrationEvent>
{
    public async Task Consume(ConsumeContext<OrderCreatedIntegrationEvent> context)
    {
        ...
        logger.LogInformation("[{Consumer}] Sending email to {Email} for order {OrderId}",
            nameof(OrderCreatedIntegrationEventHandler), @event.Email, @event.OrderId);
