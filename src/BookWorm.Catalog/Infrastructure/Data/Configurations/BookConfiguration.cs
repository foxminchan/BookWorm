...
internal sealed class BookConfiguration : BaseConfiguration<Book>
{
    public override void Configure(EntityTypeBuilder<Book> builder)
    {
        ...
        builder.OwnsOne(p => p.Price, e => e.ToJson());

        builder.Property(x => x.ImageUrl).HasMaxLength(DataSchemaLength.SuperLarge);

        builder.Property(p => p.AverageRating).HasDefaultValue(0.0);

        builder.Property(p => p.TotalReviews).HasDefaultValue(0);

        builder.Property(p => p.Embedding).HasColumnType(VectorType.Type);
        ...
        builder.HasMany(x => x.BookAuthors).WithOne(x => x.Book);
