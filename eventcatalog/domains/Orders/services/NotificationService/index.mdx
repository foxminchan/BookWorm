...
C4Component
    ...
    Container_Boundary(notification, "Notification Service") {
        ...
        Component(notificationManager, "Notification Manager", ".NET", "Core business logic for notifications")
        Component(smtpClient, "SMTP Client", ".NET", "Email delivery abstraction")
        ContainerDb(notificationDb, "Notification DB", "Azure Table Storage", "Stores notification records")
    }

    SystemQueue(eventBus, "Event Bus", "Handles async communication")
    System_Ext(sendGrid, "SendGrid", "Production email provider")
    System_Ext(mailPit, "MailPit", "Development email provider")

    Rel(eventBus, eventHandler, "Events", "Async")
    Rel(eventHandler, notificationManager, "Uses", "Internal")
    Rel(notificationManager, emailService, "Uses", "Internal")
    ...
    Rel(smtpClient, mailPit, "Sends emails (Dev)", "SMTP")
    Rel(notificationManager, notificationDb, "Reads/Writes", "TCP")
