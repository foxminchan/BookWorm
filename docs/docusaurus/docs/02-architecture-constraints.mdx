...
description:
  Technical, organizational, and convention constraints defining boundaries and conditions for
  BookWorm system design
keywords:
  [
...
Architecture constraints define the boundaries and non-negotiable conditions under which the
BookWorm system must be designed and implemented. They ensure alignment across teams, reduce
architectural drift, and make explicit the trade-offs that were accepted.
...
| ------------------------ | ---------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------- |
| **.NET 10**              | Target framework (net10.0)         | Latest .NET with enhanced performance, C# preview features, native AOT support         | Modern runtime with cutting-edge capabilities                  | Low        | Requires staying current with .NET releases, preview features may change   |
| **C# Preview Features**  | LangVersion=preview                | Access to latest language innovations and patterns                                     | Modern syntax and improved developer productivity              | Medium     | Preview features may change before final release                           |
...
| **Kafka + WolverineFx**  | Message broker + async messaging   | Durable, ordered event streams; WolverineFx outbox/inbox for transactional consistency | Enables event-driven microservices with at-least-once delivery | Medium     | Operational overhead (Kafka cluster), schema evolution discipline required |
...
<p style={{ textAlign: "justify" }}>
  These architecture constraints establish the <b>non-negotiable foundation</b> of the system while
  making clear which areas are <b>flexible</b>. They balance innovation (e.g., Aspire, Qdrant) with
  proven practices (e.g., PostgreSQL, DDD), and provide guardrails for teams to build consistently
  and securely within agreed boundaries.
</p>
