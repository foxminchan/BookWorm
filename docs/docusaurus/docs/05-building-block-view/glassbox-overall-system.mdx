...
C4Container
    ...
    Enterprise_Boundary(bookworm, "BookWorm Store") {
      ...
      System_Boundary(infrastructure, "Infrastructure") {
        SystemQueue(eventBus, "Event Bus", "Kafka / WolverineFx")
    ...
    BiRel(catalogSvc, eventBus, "Publishes/Subscribes", "Kafka")
    BiRel(orderSvc, eventBus, "Publishes/Subscribes", "Kafka")
    BiRel(basketSvc, eventBus, "Publishes/Subscribes", "Kafka")
    BiRel(financeSvc, eventBus, "Publishes/Subscribes", "Kafka")
    BiRel(ratingSvc, eventBus, "Publishes/Subscribes", "Kafka")
    Rel(eventBus, notificationSvc, "Subscribes", "Kafka")
    Rel(eventBus, scheduler, "Subscribes", "Kafka")
...
| Event Bus            | Kafka        | Asynchronous messaging for domain events               |
...
- Event-driven architecture with Kafka + WolverineFx
