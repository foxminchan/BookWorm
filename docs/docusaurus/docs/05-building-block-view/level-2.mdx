...
C4Component
    ...
    Container_Boundary(catalog, "Catalog Service") {
        ...
        Container_Boundary(infrastructure, "Infrastructure") {
            ...
            ComponentQueue(eventBus, "Event Bus", "Kafka", "Handles async communication (WolverineFx)")
...
C4Component
    ...
    Container_Boundary(chat, "Chat Service") {
        ...
        Container_Boundary(infrastructure, "Infrastructure") {
            ...
            Container_Boundary(ai, "AI") {
                Component(azureOpenAI, "Azure OpenAI", "Managed AI Service", "GPT-4o-mini for conversations")
                Component(mcp, "MCP Server", "MCP C# SDK", "Get and run tools")

                Container_Boundary(model, "Model") {
                    Component(chatModel, "GPT-4o-mini", "Chat Model", "Handles chat interactions")
                }
            }
        }
    }
    ...
    Rel(azureOpenAI, chatModel, "Uses", "Internal")
...
C4Component
    ...
    Container_Boundary(notification, "Notification Service") {
        ...
        Container_Boundary(infrastructure, "Infrastructure") {
            ComponentDb(notificationDb, "Notification DB", "PostgreSQL", "Stores notification records")
            ComponentQueue(eventBus, "Event Bus", "Kafka", "Handles async communication (WolverineFx)")
        }
    }
...
C4Container
    ...
    Container_Boundary(order, "Order Service") {
        ...
        Component(saga, "Saga Orchestrator", "WolverineFx", "Distributed transaction coordination")
    ...
    SystemQueue(eventBus, "Event Bus", "Kafka", "Domain events")
    ...
    Rel(saga, eventBus, "Publishes/Subscribes", "Kafka")
