...
#### Message Broker (Kafka + WolverineFx)

- **Protocol**: Apache Kafka binary protocol on port 9092 (PLAINTEXT/SASL)
- **Messaging Library**: WolverineFx with `WolverineFx.Kafka` transport
- **Delivery**: At-least-once delivery with durable outbox (PostgreSQL-backed) for transactional
  consistency
- **Patterns**: Publish/subscribe via Kafka topics; Wolverine inbox for idempotent consumers
- **Dev Environment**: Kafka + KafkaUI via Aspire (`AddKafka().WithKafkaUI()`)
...
graph TB
    ...
    Services -->|TCP:9092| MessageBroker
...
- **Message Handling**: WolverineFx with Kafka (outbox/inbox patterns)
