...
| **Asynchronous** | Kafka with WolverineFx | Reliable message delivery, event-driven architecture, decoupling         |
| **Streaming AI** | Server-Sent Events     | Token-by-token streaming of LLM responses to the storefront (CopilotKit) |
...
- **Command Processing**:
  - _In-process_: Mediator for local command handling.
  - _Cross-service_: **Kafka with WolverineFx** for distributed commands and integration events.
...
- **Saga Orchestration**:
  - Manage long-running or multi-service workflows using **WolverineFx** (saga orchestration with
    Kafka transport and PostgreSQL-backed outbox/inbox).
