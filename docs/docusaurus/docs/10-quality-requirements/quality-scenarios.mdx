...
description:
  Concrete quality scenarios demonstrating expected system behavior under specific conditions
keywords:
  [
    quality scenarios,
...
Quality scenarios provide concrete examples of how BookWorm should behave under specific conditions.
Each scenario follows the standard format: Source → Stimulus → Environment → Artifact → Response →
Response Measure.
...
- **Environment**: Production with horizontally scaled Chat service
- **Artifact**: Chat service and SSE streaming connections
...
- **Source**: AI service (Azure OpenAI) experiencing high latency
