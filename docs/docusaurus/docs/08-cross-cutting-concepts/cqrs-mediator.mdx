...
title: CQRS & Mediator
description:
  CQRS pattern implementation with Mediator for command query separation and pipeline behaviors
keywords:
  [CQRS, Mediator, command query separation, pipeline behaviors, validation, logging, handlers]
last_update:
  date: "2026-05-19"
  author: Nhan Nguyen
---
...
The BookWorm application implements Command Query Responsibility Segregation (CQRS) pattern using
Mediator, providing clear separation between read and write operations with comprehensive pipeline
...
- **`ICommand<T>` / `IQuery<T>`** - Base contracts for commands and queries (source-generated via
  `Mediator.SourceGenerator`)
- **`ICommandHandler<TCommand, TResult>` / `IQueryHandler<TQuery, TResult>`** - Handler
  implementation contracts
- **Dependency Injection** - Automatic handler discovery and registration at compile time
