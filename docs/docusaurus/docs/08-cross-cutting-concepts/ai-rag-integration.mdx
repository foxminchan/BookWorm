...
description:
  AI capabilities with Agent Framework, RAG, and Model Context Protocol for intelligent features and
  automation
keywords:
  [
    AI integration,
...
The BookWorm application integrates advanced AI capabilities through Agent Framework,
Retrieval-Augmented Generation (RAG), and agent-based architectures to provide intelligent features
and automated decision-making across the system.
...
- **Azure OpenAI Integration** - Managed LLM service (GPT-4o-mini) and embeddings
...
- **Model Selection** - GPT-4o-mini for conversational AI; text-embedding-3-large (3072 dims) for
  embeddings
- **Responses & Conversations APIs** - `AddOpenAIResponses()` and `AddOpenAIConversations()` for
  stateful chat
- **Token Management** - Per-request token budgets and usage telemetry via OpenTelemetry
- **Cost Control** - Centralized provisioning via Aspire and Azure Key Vault for keys

### Chat Completion Service

- **Streaming Responses** - Server-Sent Events stream tokens to the storefront (CopilotKit)
- **Context Management** - Maintain conversation history via Conversations API
- **Temperature Control** - Adjust response creativity and consistency
- **Token Management** - Optimize token usage and response length

### Embedding Service

- **Text Embeddings** - Generate 3072-dimensional vectors via `text-embedding-3-large`
- **Batch Processing** - Process multiple texts efficiently
- **Catalog Indexing** - Embeddings stored in Qdrant for semantic book search
- **Model Selection** - Standardized on Azure OpenAI for production parity
