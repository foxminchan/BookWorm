...
- **Event-Driven Architecture** - Integration events with WolverineFx and Kafka
...
- **AI Integration** - Agent Framework, Azure OpenAI, and RAG capabilities
...
BookWorm uses **FusionCache** (`ZiggyCreatures.FusionCache`) for a two-level (L1 + L2) caching
strategy, combining in-memory and distributed caching for optimal performance.

### Why FusionCache?

- **L1 (In-Memory)**: Fast local cache with microsecond access times
- **L2 (Redis)**: Distributed cache shared across service instances
- **Automatic Coordination**: FusionCache manages L1/L2 synchronization automatically
- **Stampede Protection**: Built-in protection against cache stampede scenarios
- **Resiliency**: Fail-safe, soft/hard timeouts, eager refresh, and auto-recovery
...
public class BookService
{
    ...
    public async Task<Book> GetBookAsync(BookId id, CancellationToken ct)
    {
        return await _cache.GetOrSetAsync(
            $"book:{id}",
            async (_, cancel) => await _repository.GetByIdAsync(id, cancel),
            token: ct
        );
    }
}
