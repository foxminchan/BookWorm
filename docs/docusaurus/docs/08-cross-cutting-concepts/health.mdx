...
title: Health Checks
description:
  Comprehensive health monitoring with ASP.NET Core health checks for service availability and
  dependencies
keywords:
  [
    health checks,
...
The BookWorm application implements comprehensive health monitoring using ASP.NET Core health
checks, providing real-time insights into service availability and dependencies.
...
- **Kafka Connectivity** - Broker connection status (WolverineFx)
- **Topic / Consumer Group Status** - Consumer lag and processing rates
- **Publisher Confirmation** - Message delivery confirmation via outbox
