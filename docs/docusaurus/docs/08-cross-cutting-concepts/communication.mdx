...
title: Communication
description:
  Synchronous and asynchronous communication patterns between microservices using HTTP, gRPC, and
  messaging
keywords:
  [
    communication,
...
The BookWorm application implements multiple communication patterns to support both synchronous and
...
#### Message Bus (WolverineFx + Kafka)

- **Integration Events** - Domain events for cross-service communication
- **Event Dispatching** - Reliable message delivery and processing via CloudEvents envelope
- **Message Patterns** - Support for publish/subscribe, saga orchestration, and outbox/inbox
  patterns
- **Dead Letter Handling** - Native Kafka dead-letter queue with `EnableNativeDeadLetterQueue()`
