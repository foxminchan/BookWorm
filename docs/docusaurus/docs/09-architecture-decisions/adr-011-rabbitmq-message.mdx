...
keywords:
  [ADR-011, RabbitMQ, message broker, asynchronous messaging, MassTransit, event bus, messaging]
status: Superseded
...
:::warning Superseded by [ADR-022](./adr-022-wolverinefx-kafka) — May 2026 BookWorm migrated from
RabbitMQ + MassTransit to **Kafka + WolverineFx**. The Kafka-based stack provides durable, ordered
event streams with WolverineFx outbox/inbox for transactional consistency. This ADR is kept for
...
Adopt **RabbitMQ** as the primary message broker, integrated with **MassTransit** for .NET-based
