...
title: Overview
description:
  Architecture Decision Records (ADRs) documenting significant architectural choices and their
  rationale for BookWorm
keywords:
  [
    ADR,
...
This section documents significant architectural decisions made for the BookWorm project using
Architecture Decision Records (ADRs). Each ADR captures the context, decision, rationale, and
consequences of important architectural choices that shape the system's design and implementation.
...
gantt
    ...
    WolverineFx + Kafka    :done, data4, 2026-05-01, 19d
    ...
    Ollama Runtime         :crit, ai2, 2025-03-15, 30d
...
| [ADR-010](./adr-010-redis-signalr-scale)    | Redis for SignalR Scaling      | Deprecated | Medium |
| [ADR-011](./adr-011-rabbitmq-message)       | RabbitMQ for Message Broker    | Superseded | High   |
| [ADR-013](./adr-013-ordering-eventsoucring) | Event Sourcing for Ordering    | Accepted   | Medium |
| [ADR-022](./adr-022-wolverinefx-kafka)      | WolverineFx + Kafka Messaging  | Accepted   | High   |
...
| ADR                                          | Title                               | Status     | Impact |
| -------------------------------------------- | ----------------------------------- | ---------- | ------ |
| [ADR-006](./adr-006-signalr-realtime)        | SignalR for Real-time Communication | Deprecated | Medium |
...
| ADR                                         | Title                             | Status     | Impact |
| ------------------------------------------- | --------------------------------- | ---------- | ------ |
| [ADR-009](./adr-009-ai-integration)         | Azure OpenAI Integration Strategy | Accepted   | High   |
| [ADR-012](./adr-012-ollama-llm-runtime)     | Ollama for LLM Runtime            | Deprecated | Medium |
| [ADR-020](./adr-020-mcp-tools-standardize)  | MCP Tools for AI Standardization  | Accepted   | Medium |
| [ADR-021](./adr-021-agent-to-agent-via-a2a) | Agent-to-Agent A2A Protocol       | Accepted   | Medium |
...
graph TD
    ...
    B --> D[ADR-022: WolverineFx + Kafka]
    D -->|supersedes| D011[ADR-011: RabbitMQ]
...
- [Technical Risks and Debts](../risks-and-technical-debts): Risk analysis related to architectural
  decisions
- [Quality Requirements](../quality-requirements/overview): Quality attributes addressed by ADRs
- [Building Block View](../building-block-view/glassbox-overall-system): How ADRs shape system
  components
