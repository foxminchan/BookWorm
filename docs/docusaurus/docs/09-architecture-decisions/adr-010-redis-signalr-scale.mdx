...
keywords:
  [ADR-010, Redis, SignalR, horizontal scaling, backplane, distributed caching, session management]
status: Deprecated
last_update:
  date: "2026-05-19"
  author: Nhan Nguyen
...
:::warning Deprecated — May 2026 The SignalR backplane use case for this ADR is no longer relevant
after SignalR was removed (see ADR-006). Redis remains in BookWorm as a hybrid L1/L2 cache via
FusionCache, but no longer serves as a SignalR backplane. This ADR is kept for historical reference.
:::

## Context

BookWorm's microservices architecture requires high-performance caching and real-time communication
scaling capabilities. The system faces several performance and scalability challenges:

- **Caching Requirements**: Fast data access for frequently requested catalog information, user
...
Traditional in-memory caching doesn't scale across multiple instances, while database-only
approaches can't meet the performance requirements for real-time features and high-traffic
scenarios.

## Decision

Adopt **Redis** as the primary distributed caching and session storage solution, serving both as a
high-performance cache for application data and as the backplane for SignalR horizontal scaling.
...
BookWorm uses Redis primarily for distributed state management and real-time communication scaling,
with services utilizing Redis based on their specific needs:
...
The implementation focuses on scalable real-time chat functionality rather than traditional SignalR
backplane patterns.
