...
BookWorm requires a cloud deployment platform that supports containerized microservices with
auto-scaling and service discovery. The deployment needs:
...
Use **Azure Container Apps** as the deployment platform, deployed via **Azure Developer CLI (azd)**
