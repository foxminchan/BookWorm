...
keywords:
  [ADR-004, PostgreSQL, database, ACID, JSON support, relational database, Entity Framework Core]
last_update:
  author: Nhan Nguyen
---
...
BookWorm's microservices architecture requires a robust, scalable, and feature-rich database
solution that can support diverse data patterns across multiple services. The system has specific
requirements:
...
The database choice significantly impacts development velocity, operational complexity, and
long-term maintainability of the entire system.

## Decision

Adopt **PostgreSQL** as the primary relational database for all services requiring persistent data
storage, with specialized data stores (Redis, Qdrant) for specific use cases.
