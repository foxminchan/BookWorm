...
description:
  Implementation of CI/CD pipeline with GitHub Actions for automated testing and deployment
keywords:
  [ADR-015, GitHub Actions, CI/CD, continuous integration, continuous deployment, automation]
last_update:
  date: "2026-05-19"
  author: Nhan Nguyen
---
...
BookWorm's microservices architecture requires a robust continuous integration and deployment
pipeline to support rapid development, automated testing, and reliable deployments across multiple
environments. The CI/CD requirements include:
...
- **Security Scanning**: Static code analysis, dependency vulnerability scanning, and secrets
  detection
- **Container Management**: Docker image building, scanning, and registry management
- **Environment Promotion**: Automated deployment to development, staging, and production
  environments
...
The CI/CD solution must integrate seamlessly with GitHub repositories while providing
enterprise-grade features for a production microservices architecture.

## Decision

Adopt **GitHub Actions** as the primary CI/CD platform with custom workflows for each microservice
and shared reusable workflows for common operations, integrated with Azure Container Registry and
deployment targets.
...
2. **Parallel Quality Checks**:
   - Security scanning (Snyk, Betterleaks)
...
4. **Container Publishing**: Multi-service Docker image publishing to Azure Container Registry
...
1. **Repository Integration**: Native integration with GitHub repositories, issues, and pull
   requests
...
2. **Matrix Strategies**: Parallel execution across multiple dimensions (services, environments,
   versions)
...
1. **Concurrency Management**: Pipeline concurrency control with automatic cancellation of outdated
   runs
2. **Smart Path Filtering**: Workflow triggers based on specific file patterns (src/**, tests/**,
   configuration files)
3. **Dependency Caching**: Comprehensive caching for .NET packages, SDK installations, and build
   artifacts
...
2. **Security Scanning**: Snyk for vulnerability scanning and dependency analysis, Betterleaks for
   secrets detection
3. **Container Management**: Azure Container Registry (ACR) for image storage with automated tagging
...
│   ├── security-scan/              # Betterleaks secrets detection
│   ├── format/                     # Code formatting validation
│   ├── style/                      # Style checking
│   ├── analyzers/                  # Static code analysis
│   └── publish/                    # Container publication is handled by ACR
└── dependencies/
...
```yaml
# Services published to Azure Container Registry
services:
...
- **Betterleaks secret scanning** with non-blocking warnings for detecting hardcoded secrets
...
security_scanning:
  betterleaks:
    version: "v1.1.1" # Specific version pinning
