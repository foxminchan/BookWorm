...
:::warning Deprecated — May 2026 SignalR was removed from BookWorm. Real-time chat is now handled by
Server-Sent Events (SSE) via ASP.NET Core streaming endpoints, and push notifications are delivered
through WolverineFx event handlers. This ADR is kept for historical reference. :::
