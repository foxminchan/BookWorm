...
:::warning Deprecated — May 2026 Ollama was replaced by **Azure OpenAI** (GPT-4o-mini for chat,
text-embedding-3-large for embeddings) to leverage enterprise SLA, managed scaling, and Microsoft
ecosystem integration. Local model hosting via Ollama is no longer used in production. This ADR is
