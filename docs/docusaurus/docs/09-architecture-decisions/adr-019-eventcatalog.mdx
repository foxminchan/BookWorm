...
description:
  Use of EventCatalog for documenting event-driven architecture and AsyncAPI specifications
keywords:
  [ADR-019, EventCatalog, event documentation, AsyncAPI, event-driven architecture, event schemas]
last_update:
  author: Nhan Nguyen
---
...
BookWorm's event-driven microservices architecture produces numerous domain events, integration
events, and system events that require comprehensive documentation and discoverability. The event
...
- **Producer-Consumer Relationships**: Understanding which services produce and consume specific
  events
...
Adopt **EventCatalog** as the primary platform for documenting, discovering, and managing events
across BookWorm's event-driven architecture, integrated with code generation and CI/CD pipelines.
