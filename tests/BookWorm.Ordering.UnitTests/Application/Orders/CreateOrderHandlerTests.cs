...
public sealed class CreateOrderHandlerTests
{
    ...
    {
        _handler = new(_repositoryMock.Object, _basketServiceMock.Object, _identityServiceMock.Object,
    ...
    public async Task GivenNullBuyerId_ShouldThrowArgumentException_WhenOrderIsCreated()
    {
        ...
        _publishEndpointMock.Verify(
            p => p.Publish(It.IsAny<OrderCreatedIntegrationEvent>(), It.IsAny<CancellationToken>()), Times.Never);
    }
        ...
        [CombinatorialValues(true, false)] bool isBasketNull)
    {
        ...
        var basket = isBasketNull
            ? null
            : new Grpc.Basket(Guid.NewGuid(),
                new List<BasketItem>
                {
                    new(Guid.NewGuid(), "Book 1", 2, 10.0m, 8.0m), new(Guid.NewGuid(), "Book 2", 1, 20.0m, 18.0m)
                }, 26.0m);

        _identityServiceMock.Setup(s => s.GetUserIdentity()).Returns(Guid.NewGuid().ToString());
        _basketServiceMock.Setup(s => s.GetBasketAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))!
            .ReturnsAsync(basket);
        ...
        else
        {
            ...
            _publishEndpointMock.Verify(
                p => p.Publish(It.IsAny<OrderCreatedIntegrationEvent>(), It.IsAny<CancellationToken>()), Times.Once);
        }
    }
}
